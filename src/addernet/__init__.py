...
def _ensure_native_files() -> None:
    ...
    try:
        ...
        raise OSError(
            "AdderNet could not compile its native CPU libraries. Install gcc/clang "
            "or use a compatible prebuilt wheel."
        ) from exc
    missing = [name for name in (layer_name, hdc_name) if not (_HERE / name).exists()]
    if missing:
...
from .models import AdderNetClassifier, AdderNetMultiInputLayer, AdderNetRegressor
from .preprocessing import UniformQuantizer
from .reference import ReferenceAdderNetLayer
...
__all__ = [
    "AdderNetLayer", "AdderNetMultiInputLayer", "AdderNetRegressor",
    "AdderNetClassifier", "AdderNetHDC", "AnHdcModel", "hdc_detect_backend",
